...
contract Doacao {
    ...
    function donateMPE1(address  recepient, uint256 value) public {
        if (msg.sender== owner) {
            tokenMPE1.transfer(recepient,value);
        }
    }
  
    function donateMPE2 (address  recepient, uint256 value) public {
        if (msg.sender== owner) {
    ...
    fuction convertCents (uint256 value, uint256 cents, uint decimals) internal view returns (uint256) {
        uint256 finalValue =         
    }
  
  function donateEther (address payable recepient, uint256 value, uint256 cent) external {
